...
def convert_to_wind_components(theta: float, speed: float) -> Tuple[float, float]:
    """Convert wind speed given by an angle, in degrees, and speed to vector components
    """
    theta = np.deg2rad(theta)
    u_wind = -speed * np.sin(theta)
    v_wind = -speed * np.cos(theta)
...
def build_asos_request_url(station, start_date=None, end_date=None):
    ...
    end_data: datetime.datetime
        Ending time of data to be obtained - defaults to now
...
def download_asos_data(url) -> pd.DataFrame:
    """
